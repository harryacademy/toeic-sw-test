...
- 0-200 estimate (owner approved 2026-09-25, shown to students): Score.gs. Weighted per-type averages (0.20/0.35/0.45) -> piecewise-linear points fitted to the ETS manual's example table -> rounded to 10, shown +/-10 with ETS level. All numbers in the ScoreMap Sheet tab (seeded by setupSheets, never overwritten). testScoreMap checks the 9 ETS example rows.
- Anchors (2026-09-25): primarily rows in the Anchors Sheet tab (Anchors.gs), created from Results rows via menu 'Tạo anchor từ dòng đang chọn', used when active + owner_score. PrivateAnchors.gs is optional (holds the one-time seedMyFirstAnchors). Owner anchors: W8-6172bbc8=4, W6-33a02039=3 (questions not explained, simple sentences), W8-33a02039=5.
- Q6-7 house rule refined (2026-09-25): each question/suggestion needs its own reason; opening background does not count; bare question -> max 3. Temp test after spelling rule: essays 4 and 5 matched owner 6/6; W6 got 4 vs owner 3 (no other email anchor), hence this refinement. T=1.0 and 0.2 identical -> keep 1.0.
- Global SPELLING rule in the grader prompt: spelling/obvious typos never lower a score; band_note must not cite them (owner, after AI gave a 5-level essay 4 for spelling).
...
- 2026-09-25 (end): backend @9. Anchors tab live with 4 owner anchors (W8-6172bbc8=4, W6-33a02039=3, W7-33a02039=4, W8-33a02039=5). Temperature test: 24/24 exact at both T=1.0 and 0.2 (leave-one-out). Caveat: all anchors written by the owner on one sample form; needs real student anchors. Phase 1 functionally complete; waiting for owner sign-off before planning Phase 2.
